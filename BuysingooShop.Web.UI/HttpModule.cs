...
namespace BuysingooShop.Web.UI
{
    ...
    public class HttpModule : System.Web.IHttpModule
    {
        ...
        private void ReUrl_BeginRequest(object sender, EventArgs e)
        {
            ...
            context.RewritePath(string.Format("{0}{1}/{2}{3}", siteConfig.webpath, Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_ASPX, Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_PLUGIN, requestPath));
            return;

            //如果虚拟目录(不含安装目录)与站点根目录名相同则不需要重写
            //if (IsDirExist(Vincent._DTcms.DTKeys.CACHE_SITE_DIRECTORY, siteConfig.webpath, siteConfig.webpath, requestPath))
            //{
            //    return;
            //}
            ...
            if (siteConfig.staticstatus == 0)
            {
                ...
                foreach (Model.url_rewrite model in SiteUrls.GetSiteUrls().Urls)
                {
                    ...
                    if (model.page == requestPath.Substring(requestPath.LastIndexOf("/") + 1))
                    {
                        ...
                        if (model.type == Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_PLUGIN)
                        {
                            context.RewritePath(string.Format("{0}{1}/{2}{3}", 
                                siteConfig.webpath, Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_ASPX, Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_PLUGIN, requestPath));
                            return;
                        }
                        else
                        {
                            ...
                            context.RewritePath(string.Format("{0}{1}/{2}{3}",
                                siteConfig.webpath, Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_ASPX, defaultPath, requestPath));
                            return;
                        }
                    }
                }
            ...
            else
            {
                ...
                foreach (Model.url_rewrite model in SiteUrls.GetSiteUrls().Urls)
                {
                    ...
                    if (model.url_rewrite_items.Count == 0 &&
                        Vincent._DTcms.Utils.GetUrlExtension(model.page, siteConfig.staticextension) == requestPath.Substring(requestPath.LastIndexOf("/") + 1))
                    {
                        //如果该页面属于插件页则映射到插件目录，否则映射到频道分类目录
                        if (model.type == Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_PLUGIN)
                        {
                            context.RewritePath(string.Format("{0}{1}/{2}/{3}",
                                siteConfig.webpath, Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_ASPX, Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_PLUGIN, model.page));
                            return;
                        }
                        else
                        {
                            ...
                            context.RewritePath(string.Format("{0}{1}/{2}/{3}", siteConfig.webpath, Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_ASPX, defaultPath, model.page));
                            return;
                        }
                    }
                    //遍历URL字典的子节点
                    foreach (Model.url_rewrite_item item in model.url_rewrite_items)
                    {
                        ...
                        string newPattern = Vincent._DTcms.Utils.GetUrlExtension(item.pattern, siteConfig.staticextension); //替换扩展名
                            ...
                            || (model.page == "index.aspx" && Regex.IsMatch(requestPath, string.Format("^{0}{1}$", patternPath, item.pattern), RegexOptions.None | RegexOptions.IgnoreCase)))
                        {
                            ...
                            if (siteConfig.staticstatus == 2 && defaultPath != Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_MOBILE && 
                                (model.channel.Length > 0 || model.page.ToLower() == "index.aspx")) //频道页
                            {
                                if (domainPath == string.Empty && categoryPath != string.Empty)
                                {
                                    context.RewritePath(siteConfig.webpath + Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_HTML + Vincent._DTcms.Utils.GetUrlExtension(requestPath, siteConfig.staticextension, true));
                                }
                                else
                                {
                                    context.RewritePath(siteConfig.webpath + Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_HTML + "/" + defaultPath + Vincent._DTcms.Utils.GetUrlExtension(requestPath, siteConfig.staticextension, true));
                                }
                                return;
                            }
                            else if (model.type == Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_PLUGIN) //插件页
                            {
                                ...
                                context.RewritePath(string.Format("{0}{1}/{2}/{3}", siteConfig.webpath, Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_ASPX, Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_PLUGIN, model.page), 
                            ...
                            else //其它
                            {
                                string queryString = Regex.Replace(requestPath, string.Format("{0}{1}", patternPath, newPattern),
                                    item.querystring, RegexOptions.None | RegexOptions.IgnoreCase);
                                context.RewritePath(string.Format("{0}{1}/{2}/{3}",
                                    siteConfig.webpath, Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_ASPX, defaultPath, model.page), string.Empty, queryString);
                                return;
                            }
        ...
        {
            ArrayList _cache = _Cache.Get<ArrayList>(cacheKey); //从续存中取
            if (_cache == null)
            {
                _cache = new ArrayList();
                DirectoryInfo dirInfo = new DirectoryInfo(Vincent._DTcms.Utils.GetMapPath(dirPath));
                foreach (DirectoryInfo dir in dirInfo.GetDirectories())
                {
                    _cache.Add(dir.Name.ToLower());
                }
                _Cache.Insert(cacheKey, _cache, 2); //存入续存，弹性2分钟
            }
            return _cache;
        }
    ...
    public class SiteUrls
    {
        ...
        {
            SiteUrls _cache = _Cache.Get<SiteUrls>(Vincent._DTcms.DTKeys.CACHE_SITE_HTTP_MODULE);
            lock (lockHelper)
            {
                if (_cache == null)
                {
                    _Cache.Insert(Vincent._DTcms.DTKeys.CACHE_SITE_HTTP_MODULE, new SiteUrls(), Vincent._DTcms.Utils.GetXmlMapPath(Vincent._DTcms.DTKeys.FILE_URL_XML_CONFING));
                    instance = _Cache.Get<SiteUrls>(Vincent._DTcms.DTKeys.CACHE_SITE_HTTP_MODULE);
                }
            }
            return instance;
        }
