...
namespace BuysingooShop.Web.UI
{
    public class UpLoad
    {
        ...
        {
            string fileExt = Vincent._DTcms.Utils.GetFileExt(fileName); //文件扩展名，不含“.”
            if (!IsImage(fileExt))
            {
                return false;
            }
            string newFileDir = Vincent._DTcms.Utils.GetMapPath(newFileName.Substring(0, newFileName.LastIndexOf(@"/") + 1));
            //检查是否有该路径，没有则创建
            ...
            try
            {
                string fileFullPath = Vincent._DTcms.Utils.GetMapPath(fileName);
                string toFileFullPath = Vincent._DTcms.Utils.GetMapPath(newFileName);
                return Vincent._DTcms.Thumbnail.MakeThumbnailImage(fileFullPath, toFileFullPath, 180, 180, cropWidth, cropHeight, X, Y);
            }
            catch
            {
                return false;
        ...
        public string fileSaveAs(HttpPostedFile postedFile, bool isThumbnail, bool isWater)
        {
            try
            {
                ...
                {
                    Vincent._DTcms.Thumbnail.MakeThumbnailImage(fullUpLoadPath + newFileName, fullUpLoadPath + newThumbnailFileName,
                ...
                if (IsWaterMark(fileExt) && isWater)
                {
                    ...
                    {
                        case 1:
                            Vincent._DTcms.WaterMark.AddImageSignText(newFilePath, newFilePath,
                            ...
                            break;
                        case 2:
                            Vincent._DTcms.WaterMark.AddImageSignPic(newFilePath, newFilePath,
