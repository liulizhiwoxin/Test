...
namespace BuysingooShop.Web.UI.Page
{
    public partial class oauth_login : Web.UI.BasePage
    {
        ...
        {
            turl = Vincent._DTcms.Utils.GetCookie(Vincent._DTcms.DTKeys.COOKIE_URL_REFERRER);
            ...
            {
                HttpContext.Current.Response.Redirect(linkurl("error", "?msg=" + Vincent._DTcms.Utils.UrlEncode("登录失败，用户授权已过期，请重新登录！")));
                return;
            }
            ...
            if (oauthModel != null)
            {
                ...
                if (model == null)
                {
                    HttpContext.Current.Response.Redirect(linkurl("error", "?msg=" + Vincent._DTcms.Utils.UrlEncode("登录失败，授权用户不存在或已被删除！")));
                    return;
                }
                ...
                Vincent._DTcms.Utils.WriteCookie(Vincent._DTcms.DTKeys.COOKIE_USER_NAME_REMEMBER, "DTcms", model.user_name);
                Vincent._DTcms.Utils.WriteCookie(Vincent._DTcms.DTKeys.COOKIE_USER_PWD_REMEMBER, "DTcms", model.password);
                //更新最新的Access Token
