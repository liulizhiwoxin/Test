...
namespace BuysingooShop.Web.UI.Page
{
    public partial class payment : Web.UI.BasePage
    {
        ...
        {
            //取得处事类型
            action = Vincent._DTcms.DTRequest.GetString("action");
            order_no = Vincent._DTcms.DTRequest.GetString("order_no");
            if (order_no.ToUpper().StartsWith("R")) //充值订单
            {
                order_type = Vincent._DTcms.DTEnums.AmountTypeEnum.Recharge.ToString().ToLower();
            }
            else if (order_no.ToUpper().StartsWith("B")) //商品订单
            {
                order_type = Vincent._DTcms.DTEnums.AmountTypeEnum.BuyGoods.ToString().ToLower();
            }
            ...
            switch (action)
            {
                case "confirm":
                    ...
                    {
                        HttpContext.Current.Response.Redirect(linkurl("error", "?msg=" + Vincent._DTcms.Utils.UrlEncode("出错啦，URL传输参数有误！")));
                        return;
                    }
                    //是否需要支持匿名购物
                    ...
                    if (order_no.ToUpper().StartsWith("R")) //充值订单
                    {
                        ...
                        if (amountModel == null)
                        {
                            HttpContext.Current.Response.Redirect(linkurl("error", "?msg=" + Vincent._DTcms.Utils.UrlEncode("出错啦，订单号不存在或已删除！")));
                            return;
                        }
                        //检查订单号是否已支付
                        ...
                        if (payModel == null)
                        {
                            HttpContext.Current.Response.Redirect(linkurl("error", "?msg=" + Vincent._DTcms.Utils.UrlEncode("出错啦，支付方式不存在或已删除！")));
                            return;
                        }
                        //检查是否线上支付
                        if (payModel.type == 2)
                        {
                            HttpContext.Current.Response.Redirect(linkurl("error", "?msg=" + Vincent._DTcms.Utils.UrlEncode("出错啦，账户充值不允许线下支付！")));
                            return;
                        }
                    ...
                    {
                        HttpContext.Current.Response.Redirect(linkurl("error", "?msg=" + Vincent._DTcms.Utils.UrlEncode("出错啦，找不到您要提交的订单类型！")));
                        return;
                    }
                    break;
                case "succeed":
