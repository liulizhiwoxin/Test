...
namespace BuysingooShop.Web.UI.Page
{
    public partial class article_show : Web.UI.BasePage
    {
        ...
        {
            id = Vincent._DTcms.DTRequest.GetQueryInt("id");
            page = Vincent._DTcms.DTRequest.GetQueryString("page");
            ...
            if (id > 0) //如果ID获取到，将使用ID
            {
                if (!bll.Exists(id))
                {
                    HttpContext.Current.Response.Redirect(linkurl("error", "?msg=" + Vincent._DTcms.Utils.UrlEncode("出错啦，您要浏览的页面不存在或已删除啦！")));
                    return;
                }
