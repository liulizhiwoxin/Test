...
namespace BuysingooShop.Web.UI
{
    public class ManagePage : System.Web.UI.Page
    {
        ...
        {
            //如果Session为Null
            if (Session[Vincent._DTcms.DTKeys.SESSION_ADMIN_INFO] != null)
            {
                return true;
            }
            else
            {
                //检查Cookies
                string adminname = Vincent._DTcms.Utils.GetCookie("AdminName", "BuysingooShop");
                string adminpwd = Vincent._DTcms.Utils.GetCookie("AdminPwd", "BuysingooShop");
        ...
        public Model.manager GetAdminInfo()
        {
            if (IsAdminLogin())
            {
                Model.manager model = Session[Vincent._DTcms.DTKeys.SESSION_ADMIN_INFO] as Model.manager;
                if (model != null)
                {
                    return model;
                }
