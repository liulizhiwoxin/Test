...
namespace BuysingooShop.Web.UI
{
    public partial class BasePage : System.Web.UI.Page
    {
        ...
        //protected internal Model.users _ModelUsers = new Model.users();
        ...
        public BasePage()
        {
            ...
            {
                HttpContext.Current.Response.Redirect(config.webpath + "error.aspx?msg=" + Vincent._DTcms.Utils.UrlEncode(config.webclosereason));
                return;
            }
            ShowPage();
            //GetUserInfo2();
        ...
        {
            return Vincent._DTcms.Utils.OutPageList(pagesize, pageindex, totalcount, linkurl(_key, _params), 8);
        }
        ...
        {
            return Vincent._DTcms.Utils.OutPageList(pagesize, pageindex, totalcount, linkurl, 8);
        }
        #endregion
        ...
        {
            //如果Session为Null
            if (HttpContext.Current.Session[Vincent._DTcms.DTKeys.SESSION_USER_INFO] != null)
            {
                return true;
            }
            else
            {
                //检查Cookies
                string username = Vincent._DTcms.Utils.GetCookie(Vincent._DTcms.DTKeys.COOKIE_USER_NAME_REMEMBER, "BuysingooShop");
                string password = Vincent._DTcms.Utils.GetCookie(Vincent._DTcms.DTKeys.COOKIE_USER_PWD_REMEMBER, "BuysingooShop");
                if (username != "" && password != "")
                {
                    ...
                    if (model != null)
                    {
                        HttpContext.Current.Session[Vincent._DTcms.DTKeys.SESSION_USER_INFO] = model;
                        return true;
                    }
                }
            }
        ...
        {
            return Vincent._DTcms.Utils.GetUrlExtension(urlPage, staticExtension);
        }
