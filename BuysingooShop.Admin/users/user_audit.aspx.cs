...
namespace BuysingooShop.Admin.users
{
    public partial class user_audit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("user_audit", Vincent._DTcms.DTEnums.ActionEnum.View.ToString()); //检查权限
        ...
        private void RptBind(string _strWhere, string _orderby)
        {
            ...
            string pageUrl = Vincent._DTcms.Utils.CombUrlTxt("user_audit.aspx", "keywords={0}&page={1}",
        ...
        {
            int _pagesize;
            if (int.TryParse(Vincent._DTcms.Utils.GetCookie("user_audit_page_size"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            Response.Redirect(Vincent._DTcms.Utils.CombUrlTxt("user_audit.aspx", "keywords={0}", txtKeywords.Text));
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Vincent._DTcms.Utils.WriteCookie("user_audit_page_size", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Vincent._DTcms.Utils.CombUrlTxt("user_audit.aspx", "keywords={0}", this.keywords));
        }
