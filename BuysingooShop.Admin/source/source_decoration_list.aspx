<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="source_decoration_list.aspx.cs" Inherits="BuysingooShop.Admin.source.source_decoration_list" %>
...
<div class="toolbar-wrap">
  <div id="floatHead" class="toolbar">
    <div class="l-list">
      <ul class="icon-list">
        <li><a class="add" href="source_decoration_edit.aspx?action=<%=Vincent._DTcms.DTEnums.ActionEnum.Add %>&channel_id=<%=this.channel_id %>"><i></i><span>新增</span></a></li>
...
<ItemTemplate>
  <tr>
    ...
    <td><a href="source_decoration_edit.aspx?action=<%#Vincent._DTcms.DTEnums.ActionEnum.Edit %>&channel_id=<%#this.channel_id %>&id=<%#Eval("id")%>"><%#Eval("title")%></a></td>
    <td><%#Eval("remark")%></td>
    <td><%#string.Format("{0:g}",Eval("add_time"))%></td>
    <td><asp:TextBox ID="txtSortId" runat="server" Text='<%#Eval("sort_id")%>' CssClass="sort" onkeydown="return checkNumber(event);" /></td>
    <td align="center"><a href="source_decoration_edit.aspx?action=<%#Vincent._DTcms.DTEnums.ActionEnum.Edit %>&channel_id=<%#this.channel_id %>&id=<%#Eval("id")%>">修改</a></td>
  </tr>
</ItemTemplate>
<FooterTemplate>
...
<ItemTemplate>
    <li>
      <div class="details<%#Eval("img_url").ToString() != "" ? "" : " nopic"%>">
        ...
        <h1><span><a href="source_decoration_edit.aspx?action=<%#Vincent._DTcms.DTEnums.ActionEnum.Edit %>&channel_id=<%#this.channel_id %>&id=<%#Eval("id")%>"><%#Eval("title")%></a></span></h1>
        <div class="remark">
          <%#Eval("remark").ToString() == "" ? "暂无背景素材说明..." : Eval("remark").ToString()%>
        </div>
        <div class="tools">
        ...
        <div class="foot">
          ...
          <a href="source_decoration_edit.aspx?action=<%#Vincent._DTcms.DTEnums.ActionEnum.Edit %>&channel_id=<%#this.channel_id %>&id=<%#Eval("id")%>" title="编辑" class="edit">编辑</a>
        </div>
      </div>
    </li>
</ItemTemplate>
