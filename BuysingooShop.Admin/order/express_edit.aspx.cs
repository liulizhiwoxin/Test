...
namespace BuysingooShop.Admin.order
{
    public partial class express_edit : Web.UI.ManagePage
    {
        ...
        private void ShowInfo(int _id)
        {
            ...
            txtRemark.Text = Vincent._DTcms.Utils.ToTxt(model.remark);
            if (model.is_lock == 0)
            {
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                AddAdminLog(Vincent._DTcms.DTEnums.ActionEnum.Edit.ToString(), "修改配送方式:" + model.title); //记录日志
                result = true;
            }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                ChkAdminLevel("order_express", Vincent._DTcms.DTEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;
