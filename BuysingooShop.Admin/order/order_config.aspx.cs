...
namespace BuysingooShop.Admin.order
{
    public partial class order_config : Web.UI.ManagePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("order_config", Vincent._DTcms.DTEnums.ActionEnum.View.ToString()); //检查权限
                ShowInfo();
            }
        }
        ...
        {
            ChkAdminLevel("order_config", Vincent._DTcms.DTEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.orderconfig bll = new BLL.orderconfig();
            Model.orderconfig model = bll.loadConfig();
            try
            {
                ...
                model.confirmmsg = Vincent._DTcms.Utils.StrToInt(confirmmsg.SelectedValue, 0);
                model.confirmcallindex = confirmcallindex.Text;
                model.expressmsg = Vincent._DTcms.Utils.StrToInt(expressmsg.SelectedValue, 0);
                model.expresscallindex = expresscallindex.Text;
                model.completemsg = Vincent._DTcms.Utils.StrToInt(completemsg.SelectedValue, 0);
                model.completecallindex = completecallindex.Text;

                bll.saveConifg(model);
                AddAdminLog(Vincent._DTcms.DTEnums.ActionEnum.Edit.ToString(), "修改订单配置信息"); //记录日志
