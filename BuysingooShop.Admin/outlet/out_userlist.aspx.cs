...
{
    public partial class out_userlist : Web.UI.ManagePage
    {
        ...
        private void RptBind(string _strWhere, string _orderby)
        {
            ...
            uid = Convert.ToInt32(Vincent._DTcms.DTRequest.GetQueryInt("id"));//获取某个用户ID  
            this.rptList.DataSource = bll.GetList(this.pageSize, this.page, _strWhere + " and 1=1 and isDelete = 0  and id=  " + uid + " ", _orderby, out this.totalCount);
            this.rptList.DataBind();

            //绑定页码
            txtPageNum.Text = this.pageSize.ToString();
            string pageUrl = Vincent._DTcms.Utils.CombUrlTxt("out_userlist.aspx", "group_id={0}&keywords={1}&page={2}",
        ...
        {
            Response.Redirect(Vincent._DTcms.Utils.CombUrlTxt("out_userlist.aspx", "group_id={0}&keywords={1}",
        ...
        protected void btnSmsPost_Click(object sender, EventArgs e)
        {
            ...
            {
                Response.Redirect("out_userlist.aspx?mobiles=" + Vincent._DTcms.Utils.UrlEncode(Vincent._DTcms.Utils.DelLastComma(str.ToString())));
            }
        }
        ...
        {
            ChkAdminLevel("out_userlist", Vincent._DTcms.DTEnums.ActionEnum.Delete.ToString()); //检查权限
            int sucCount = 0;
            int errorCount = 0;
            BLL.users bll = new BLL.users();
            for (int i = 0; i < rptList.Items.Count; i++)
            ...
            JscriptMsg("删除成功" + sucCount + "条，失败" + errorCount + "条！",
                Vincent._DTcms.Utils.CombUrlTxt("out_userlist.aspx", "group_id={0}&keywords={1}", this.group_id.ToString(), this.keywords), "Success");
        }
