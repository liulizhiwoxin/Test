...
{
    public partial class outlet_edit_ : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!string.IsNullOrEmpty(_action) && _action == Vincent._DTcms.DTEnums.ActionEnum.Edit.ToString())
            {
                ...
                if (!new BLL.outlet().Exists(this.id))
                {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("outlet_edit", Vincent._DTcms.DTEnums.ActionEnum.View.ToString()); //检查权限
                ...
                TreeBindLevel("isDelete = 0 and group_id = 3"); //绑定店长
        ...
        private bool DoAdd()
        {
            ...
            model.userId = int.Parse(DropDownList1.SelectedItem.Value);
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == Vincent._DTcms.DTEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("outlet_edit", Vincent._DTcms.DTEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
                    JscriptMsg("保存过程中发生错误啦！", "", "Error");
                    return;
                }
                JscriptMsg("修改信息成功！", "outlet_list.aspx", "Success");
            }
            else //添加
            {
                ChkAdminLevel("outlet_edit", Vincent._DTcms.DTEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误啦！", "", "Error");
                    return;
                }
                JscriptMsg("添加信息成功！", "outlet_list.aspx", "Success");
            }
        }
        #endregion
