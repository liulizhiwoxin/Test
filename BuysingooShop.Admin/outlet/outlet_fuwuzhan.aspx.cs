...
{
    public partial class outlet_fuwuzhan : System.Web.UI.Page
    {

        protected int totalCount;
        protected int page;
        protected int pageSize;
        protected int brend_id;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            brend_id = Convert.ToInt32(Vincent._DTcms.DTRequest.GetQueryInt("id"));//店铺ID
            ...
            if (!Page.IsPostBack)
            {
                ...
                if (brends_id != "")
                {
                    RptBind(brends_id+  CombSqlTxt(this.status, this.payment_status, this.express_status, this.keywords, this.keyDate), "add_time desc");
                }
                else
                {
                    RptBind("" + CombSqlTxt(this.status, this.payment_status, this.express_status, this.keywords, this.keyDate), "add_time desc");
                }
        ...
        {
            int _pagesize;
            if (int.TryParse(Vincent._DTcms.Utils.GetCookie("outlet_fuwuzhan"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        #endregion
            #region 返回订单状态=============================
        protected string GetOrderServer(int _id)
        {
            string _bill_type = string.Empty;
            ...
            DataTable dt = new BLL.orders().GetOrderList(0, " t1.id=" + _id, " id desc").Tables[0];
             ...
             //           servertype = "镀晶三档"
             //       }
            switch (int.Parse(dt.Rows[0]["bill_type"].ToString()))
            {
                case 0:  
                    _bill_type = "更换轮胎";
                    break;
        ...
        #endregion
        #region 返回订单状态=============================
        protected string GetOrderStatus2(int _id)
        {
            string _title = string.Empty;
            //Model.orders model = new BLL.orders().GetModel(_id);
            DataTable dt = new BLL.orders().GetOrderList(0, " t1.id=" + _id, " id desc").Tables[0];
        ...
        {
            Response.Redirect(Vincent._DTcms.Utils.CombUrlTxt("outlet_fuwuzhan.aspx", "status={0}&payment_status={1}&express_status={2}&keywords={3}&keyDate={4}",
        ...
        {
            //Response.Redirect(Vincent._DTcms.Utils.CombUrlTxt("outlet_fuwuzhan.aspx", "status={0}&payment_status={1}&express_status={2}&keywords={3}",
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Vincent._DTcms.Utils.WriteCookie("outlet_fuwuzhan", _pagesize.ToString(), 14400);
                }
            }
            Response.Redirect(Vincent._DTcms.Utils.CombUrlTxt("outlet_fuwuzhan.aspx", "status={0}&payment_status={1}&express_status={2}&keywords={3}",
