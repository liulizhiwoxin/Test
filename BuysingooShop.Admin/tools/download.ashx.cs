...
namespace BuysingooShop.Web.tools
{
    ...
    public class download : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (id < 1)
            {
                context.Response.Redirect(siteConfig.webpath + "error.aspx?msg=" + Vincent._DTcms.Utils.UrlEncode("出错啦，参数传值不正确哦！"));
                return;
            }
            //检查下载记录是否存在
            ...
            if (!bll.Exists(id))
            {
                context.Response.Redirect(siteConfig.webpath + "error.aspx?msg=" + Vincent._DTcms.Utils.UrlEncode("出错啦，您要下载的文件不存在或已经被删除啦！"));
                return;
            }
            ...
            if (model.point > 0)
            {
                ...
                //防止重复扣积分
                string cookie = Vincent._DTcms.Utils.GetCookie(Vincent._DTcms.DTKeys.COOKIE_DOWNLOAD_KEY, "attach_" + userModel.id.ToString());
                if (cookie != model.id.ToString())
                {
                    //检查积分
                    if (model.point > userModel.point)
                    {
                        context.Response.Redirect(siteConfig.webpath + "error.aspx?msg=" + Vincent._DTcms.Utils.UrlEncode("出错啦，您的积分不足支付本次下载！"));
                        return;
                    }
                    //扣取积分
                    new BLL.user_point_log().Add(userModel.id, userModel.user_name, model.point * -1, "下载附件：“" + model.file_name + "”，扣减积分", false);
                    //写入Cookie
                    Vincent._DTcms.Utils.WriteCookie(Vincent._DTcms.DTKeys.COOKIE_DOWNLOAD_KEY, "attach_" + userModel.id.ToString(), model.id.ToString(), 8640);
                }
            }
            //下载次数+1
            ...
            {
                //取得文件物理路径
                string fullFileName = Vincent._DTcms.Utils.GetMapPath(model.file_path);
