...
namespace BuysingooShop.Web.tools
{
    ...
    public class upload_ajax : IHttpHandler, IRequiresSessionState
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            switch (action)
            {
                case "EditorFile": //编辑器文件
                    EditorFile(context);
                    break;
                case "ManagerFile": //浏览文件处理
                    ManagerFile(context);
                    break;
                case "UpLoadFile2": //图片上传
                    UpLoadFile2(context);
                    break;
                default: //普通上传
                    UpLoadFile(context); 
        ...
        private void UpLoadFile(HttpContext context)
        {
            ...
            if (Vincent._DTcms.DTRequest.GetQueryString("IsWater") == "1")
                _iswater = true;
            if (Vincent._DTcms.DTRequest.GetQueryString("IsThumbnail") == "1")
                _isthumbnail = true;
            if (_upfile == null)
            {
        ...
        private void ManagerFile(HttpContext context)
        {
            ...
            String dirPath = Vincent._DTcms.Utils.GetMapPath(rootPath);
