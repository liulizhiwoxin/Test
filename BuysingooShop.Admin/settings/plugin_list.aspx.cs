...
namespace BuysingooShop.Admin.settings
{
    public partial class plugin_list : Web.UI.ManagePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("app_plugin_list", Vincent._DTcms.DTEnums.ActionEnum.View.ToString()); //检查权限
                RptBind();
            }
        }
        ...
        private void RptBind()
        {
            ...
            this.rptList.DataSource = bll.GetList(Vincent._DTcms.Utils.GetMapPath("../../plugins/"));
        ...
        {
            //插件目录
            string pluginPath = Vincent._DTcms.Utils.GetMapPath("../../plugins/" + dirName + "/" + Vincent._DTcms.DTKeys.FILE_PLUGIN_XML_CONFING);
            XmlNodeList xnList = _Xml.ReadNodes(pluginPath, "plugin/urls");
            if (xnList.Count > 0)
            {
                foreach (XmlElement xe in xnList)
                {
                    if (xe.NodeType != XmlNodeType.Comment && xe.Name.ToLower() == "rewrite" && xe.Attributes["page"] != null)
                    {
                        ...
                        {
                            //删除站点下的aspx文件
                            Vincent._DTcms.Utils.DeleteFile(siteConfig.webpath + "aspx/" + Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_PLUGIN + "/" + xe.Attributes["page"].Value);
                        }
                    }
                }
            }
        ...
        {
            ChkAdminLevel("app_plugin_list", Vincent._DTcms.DTEnums.ActionEnum.Instal.ToString()); //检查权限
            //插件目录
            string pluginPath = Vincent._DTcms.Utils.GetMapPath("../../plugins/");
            BLL.plugin bll = new BLL.plugin();
            //查找列表
                    ...
                    if (model.isload == 0)
                    {
                        ...
                        if (Directory.Exists(currPath))
                        {
                            ...
                            foreach (string f in file)
                            {
                                ...
                                {
                                    //移动到站点目录下
                                    string newFile = Vincent._DTcms.Utils.GetMapPath(siteConfig.webpath + @"bin\" + info.Name);
            ...
            AddAdminLog(Vincent._DTcms.DTEnums.ActionEnum.Instal.ToString(), "安装插件"); //记录日志
        ...
        {
            ChkAdminLevel("app_plugin_list", Vincent._DTcms.DTEnums.ActionEnum.UnLoad.ToString()); //检查权限
            //插件目录
            string pluginPath = Vincent._DTcms.Utils.GetMapPath("../../plugins/");
            BLL.plugin bll = new BLL.plugin();
            //查找列表
            ...
            AddAdminLog(Vincent._DTcms.DTEnums.ActionEnum.UnLoad.ToString(), "卸载插件"); //记录日志
        ...
        {
            ChkAdminLevel("app_plugin_list", Vincent._DTcms.DTEnums.ActionEnum.Build.ToString()); //检查权限
            //插件目录
            string pluginPath = Vincent._DTcms.Utils.GetMapPath("../../plugins/");
            BLL.plugin bll = new BLL.plugin();
            //查找列表
            ...
            AddAdminLog(Vincent._DTcms.DTEnums.ActionEnum.Build.ToString(), "生成插件模板"); //记录日志
