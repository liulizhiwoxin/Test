...
namespace BuysingooShop.Admin.settings
{
    public partial class templet_list : Web.UI.ManagePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("app_templet_list", Vincent._DTcms.DTEnums.ActionEnum.View.ToString()); //检查权限
                TreeBind(); //绑定频道分类
                RptBind(); //绑定模板
            }
        }
        ...
        private void RptBind()
        {
            ...
            DirectoryInfo dirInfo = new DirectoryInfo(Vincent._DTcms.Utils.GetMapPath("../../templates/"));
        ...
        private Model.template GetInfo(string xmlPath)
        {
            ...
            try
            {
                XmlNodeList xnList = _Xml.ReadNodes(xmlPath + @"\about.xml", "about");
        ...
        {
            //取得ASP目录下的所有文件
            string fullDirPath = Vincent._DTcms.Utils.GetMapPath(string.Format("{0}aspx/{1}/", siteConfig.webpath, buildPath));
            //获得URL配置列表
            ...
            {
                //如果URL配置对应的模板文件存在则生成
                string fullPath = Vincent._DTcms.Utils.GetMapPath(string.Format("{0}templates/{1}/{2}", siteConfig.webpath, skinName, modelt.templet));
                if (File.Exists(fullPath))
                {
                    Vincent._DTcms.PageTemplate.GetTemplate(siteConfig.webpath, "templates", skinName, modelt.templet, modelt.page, modelt.inherit, buildPath, 1);
                }
            }
        }
        #endregion
        ...
        protected void btnManage_Click(object sender, EventArgs e)
        {
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    Response.Redirect("templet_file_list.aspx?skin=" + Vincent._DTcms.Utils.UrlEncode(skinName));
                }
            }
        }
        ...
        {
            ChkAdminLevel("app_templet_list", Vincent._DTcms.DTEnums.ActionEnum.Build.ToString()); //检查权限
            ...
            for (int i = 0; i < rptList.Items.Count; i++)
            {
                ...
                if (cb.Checked)
                {
                    ...
                    AddAdminLog(Vincent._DTcms.DTEnums.ActionEnum.Build.ToString(), "生成模板:" + skinName);//记录日志
