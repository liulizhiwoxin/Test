...
namespace BuysingooShop.Admin.settings
{
    public partial class url_rewrite_list : Web.UI.ManagePage
    {
        ...
        {
            this.channel = Vincent._DTcms.DTRequest.GetQueryString("channel");
            this.type = Vincent._DTcms.DTRequest.GetQueryString("type");

            if (!Page.IsPostBack)
            {
                ChkAdminLevel("site_url_rewrite", Vincent._DTcms.DTEnums.ActionEnum.View.ToString()); //检查权限
                TreeBind();
        ...
        {
            Response.Redirect(Vincent._DTcms.Utils.CombUrlTxt("url_rewrite_list.aspx", "channel={0}&type={1}", ddlChannel.SelectedValue, this.type));
        }

        //筛选页面类型
        protected void ddlPageType_SelectedIndexChanged(object sender, EventArgs e)
        {
            Response.Redirect(Vincent._DTcms.Utils.CombUrlTxt("url_rewrite_list.aspx", "channel={0}&type={1}", this.channel, ddlPageType.SelectedValue));
        }

        //删除操作
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ChkAdminLevel("site_url_rewrite", Vincent._DTcms.DTEnums.ActionEnum.Delete.ToString()); //检查权限
            ...
            AddAdminLog(Vincent._DTcms.DTEnums.ActionEnum.Delete.ToString(), "删除URL配置信息"); //记录日志
