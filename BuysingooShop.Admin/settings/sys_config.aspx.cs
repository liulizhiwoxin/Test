...
namespace BuysingooShop.Admin.settings
{
    public partial class sys_config : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("site_config", Vincent._DTcms.DTEnums.ActionEnum.View.ToString()); //检查权限
                ShowInfo();
            }
        }
        ...
        private void ShowInfo()
        {
            ...
            if (model.rechangestatus == 1)
            {
                rechangestatus.Checked = true;
            }
        ...
        {
            ChkAdminLevel("site_config", Vincent._DTcms.DTEnums.ActionEnum.Edit.ToString()); //检查权限
            BLL.siteconfig bll = new BLL.siteconfig();
            Model.siteconfig model = bll.loadConfig();
            try
            {
                ...
                model.webdescription = Vincent._DTcms.Utils.DropHTML(webdescription.Text);
                model.webcopyright = webcopyright.Text;

                model.webpath = webpath.Text;
                model.webmanagepath = webmanagepath.Text;
                model.staticstatus = Vincent._DTcms.Utils.StrToInt(staticstatus.SelectedValue, 0);
                ...
                if (rechangestatus.Checked == true)
                {
                    model.rechangestatus = 1;
                }
                else
                {
                    model.rechangestatus = 0;
                }
                ...
                {
                    model.smspassword = Vincent._DTcms.Utils.MD5(smspassword.Text.Trim());
                }

                model.emailsmtp = emailsmtp.Text;
                model.emailport = Vincent._DTcms.Utils.StrToInt(emailport.Text.Trim(), 25);
                model.emailfrom = emailfrom.Text;
                model.emailusername = emailusername.Text;
                //判断密码是否更改
                if (emailpassword.Text.Trim() != defaultpassword)
                {
                    model.emailpassword = _DESEncrypt.Encrypt(emailpassword.Text, model.sysencryptstring);
                }
                model.emailnickname = emailnickname.Text;

                model.filepath = filepath.Text;
                model.filesave = Vincent._DTcms.Utils.StrToInt(filesave.SelectedValue, 2);
                model.fileextension = fileextension.Text;
                model.attachsize = Vincent._DTcms.Utils.StrToInt(attachsize.Text.Trim(), 0);
                model.imgsize = Vincent._DTcms.Utils.StrToInt(imgsize.Text.Trim(), 0);
                model.imgmaxheight = Vincent._DTcms.Utils.StrToInt(imgmaxheight.Text.Trim(), 0);
                model.imgmaxwidth = Vincent._DTcms.Utils.StrToInt(imgmaxwidth.Text.Trim(), 0);
                model.thumbnailheight = Vincent._DTcms.Utils.StrToInt(thumbnailheight.Text.Trim(), 0);
                model.thumbnailwidth = Vincent._DTcms.Utils.StrToInt(thumbnailwidth.Text.Trim(), 0);
                model.watermarktype = Vincent._DTcms.Utils.StrToInt(watermarktype.SelectedValue, 0);
                model.watermarkposition = Vincent._DTcms.Utils.StrToInt(watermarkposition.Text.Trim(), 9);
                model.watermarkimgquality = Vincent._DTcms.Utils.StrToInt(watermarkimgquality.Text.Trim(), 80);
                model.watermarkpic = watermarkpic.Text;
                model.watermarktransparency = Vincent._DTcms.Utils.StrToInt(watermarktransparency.Text.Trim(), 5);
                model.watermarktext = watermarktext.Text;
                model.watermarkfont = watermarkfont.Text;
                model.watermarkfontsize = Vincent._DTcms.Utils.StrToInt(watermarkfontsize.Text.Trim(), 12);

                bll.saveConifg(model);
                AddAdminLog(Vincent._DTcms.DTEnums.ActionEnum.Edit.ToString(), "修改系统配置信息"); //记录日志
