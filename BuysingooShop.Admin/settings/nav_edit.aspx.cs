...
namespace BuysingooShop.Admin.settings
{
    public partial class nav_edit : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("app_navigation_list", Vincent._DTcms.DTEnums.ActionEnum.View.ToString()); //检查权限
                TreeBind(Vincent._DTcms.DTEnums.NavigationEnum.System.ToString()); //绑定导航菜单
        ...
        private void ActionTypeBind()
        {
            ...
            foreach (KeyValuePair<string, string> kvp in Vincent._DTcms.Utils.ActionType())
            {
        ...
        private void ShowInfo(int _id)
        {
            ...
            txtName.Attributes.Add("ajaxurl", "../../tools/admin_ajax.ashx?action=navigation_validate&old_name=" + Vincent._DTcms.Utils.UrlEncode(model.name));
        ...
        private bool DoAdd()
        {
            try
            {
                ...
                model.nav_type = Vincent._DTcms.DTEnums.NavigationEnum.System.ToString();
                ...
                for (int i = 0; i < cblActionType.Items.Count; i++)
                {
                    if (cblActionType.Items[i].Selected && Vincent._DTcms.Utils.ActionType().ContainsKey(cblActionType.Items[i].Value))
                    {
                        action_type_str += cblActionType.Items[i].Value + ",";
                    }
                }
                model.action_type = Vincent._DTcms.Utils.DelLastComma(action_type_str);

                if (bll.Add(model) > 0)
                {
                    AddAdminLog(Vincent._DTcms.DTEnums.ActionEnum.Add.ToString(), "添加导航信息:" + model.title); //记录日志
                    return true;
                }
            }
            catch
        ...
        private bool DoEdit(int _id)
        {
            try
            {
                ...
                if (bll.Update(model))
                {
                    AddAdminLog(Vincent._DTcms.DTEnums.ActionEnum.Add.ToString(), "修改导航信息:" + model.title); //记录日志
                    return true;
                }
            }
            catch
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == Vincent._DTcms.DTEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("app_navigation_list", Vincent._DTcms.DTEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
            ...
            {
                ChkAdminLevel("app_navigation_list", Vincent._DTcms.DTEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;
