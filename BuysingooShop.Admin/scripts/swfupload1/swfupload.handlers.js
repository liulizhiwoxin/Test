...
function addImage(targetObj, originalSrc, thumbSrc) {
    ...
    + '<input type="hidden" name="hid_photo_category" value="" />'
    + '<input type="hidden" name="hid_photo_size" value="" />'
    + '<div class="img-box" onclick="setFocusImg(this);">'
    + '<img src="' + thumbSrc + '" bigsrc="' + originalSrc + '" />'
    + '<span class="remark"><i>暂无描述...</i><i>暂无主题</i><i>暂无尺寸</i></span>'
    + '</div>'
...
function setRemark(obj) {
    ...
    var hidCategoryObj = parentObj.prevAll("input[name='hid_photo_category']").eq(0);
    var hidSizeObj = parentObj.prevAll("input[name='hid_photo_size']").eq(0);

    $("#categoryID #ddlCategoryId option:selected").attr("selected", false);
    $("#categoryID #ddlCategoryId option[value='"+hidCategoryObj.val()+"']").attr("selected", true)
    var htmlCategory = "<span id='cate'>" + $("#categoryID").html() + "<span>";

    var arr = ["335*330","288*288", "297*288", "288*295", "297*295", "145*200","145*77","142*56","232*232","111*490", "313*201", "222*200", "162*298", "162*71","162*235", "163*226","41*110"];
    var htmlSize = "尺寸：<select id='img_size'><option>选择尺寸</option>";
    for (var i = 0; i < arr.length; i++) {
        if (hidSizeObj.val() == arr[i]) {
            htmlSize += "<option selected='selected'>" + arr[i] + "</option>";
        }
        else {
            htmlSize += "<option>" + arr[i] + "</option>";
        }
    }
    htmlSize += "</select><br/>";
    ...
    var m = $.dialog({
        ...
        padding: 0,
        title: "图片描述",
        content: '<br/>' + htmlCategory + htmlSize + '<textarea id="ImageRemark" style="margin:10px 0;font-size:12px;padding:3px;color:#000;border:1px #d2d2d2 solid;vertical-align:middle;width:300px;height:50px;">' + hidRemarkObj.val() + '</textarea>',
        button: [
//                {
//                    name: '批量描述',
//                    callback: function () {
//                        var remarkObj = $('#ImageRemark', parent.document);
//                        var themeObj = $("#ddlCategoryId option:selected", parent.document).text().replace("　├ ", "");
//                        var themeid = $("#ddlCategoryId option:selected", parent.document).val();
//                        var sizeObj = $("#img_size option:selected", parent.document).text();
//                        parentObj.parent().parent().find('li').each(function () {
//                            $(this).find(".img-box .remark i").eq(1).html(themeObj);
//                            $(this).find(".img-box .remark i").eq(2).html(sizeObj);
//                            $(this).find(".img-box .remark i").eq(0).html(remarkObj.val());

//                            $(this).find("input[name=hid_photo_remark]").val(remarkObj.val());
//                            $(this).find("input[name=hid_photo_category]").val(themeid);
//                            $(this).find("input[name=hid_photo_size]").val(sizeObj);
//                        });
...
//                },
                {
                    ...
                    callback: function () {
                        ...
                        var cagegoryObj = $('#ddlCategoryId option:selected', parent.document).val();
                        var cagegoryObj2 = $('#ddlCategoryId option:selected', parent.document).text().replace("　├ ", "");
                        hidCategoryObj.val(cagegoryObj);
                        var sizeObj = $('#img_size option:selected', parent.document).text();
                        hidSizeObj.val(sizeObj);
                        parentObj.siblings(".img-box").children(".remark").children("i").eq(0).html(remarkObj.val());
                        parentObj.siblings(".img-box").children(".remark").children("i").eq(1).html(cagegoryObj2);
                        parentObj.siblings(".img-box").children(".remark").children("i").eq(2).html(sizeObj);
                    },
                    focus: true
                }]
    });
