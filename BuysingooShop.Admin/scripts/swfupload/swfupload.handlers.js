$(function () {
    ...
    $.swfUpLoadDefaults.property = {
        ...
        urlparmas: "?action=UpLoadFile",//上传url地址的参数
        button_text_style:null//按钮样式
        //upload_complete: uploadComplete//上传完成之后执行的函数
    };
    //初始化SWFUpload上传控件
...
function addImage(targetObj, originalSrc, thumbSrc) {
    if ($(targetObj).prev().val() == 2) {
        //插入到相册UL里面
        var newLi = $('<li>'
    + '<input type="hidden" name="hid_photo_name1" value="0|' + originalSrc + '|' + thumbSrc + '" />'
    + '<input type="hidden" name="hid_photo_remark1" value="" />'
