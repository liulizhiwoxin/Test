<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="comment_list.aspx.cs" Inherits="BuysingooShop.Admin.article.comment_list" %>
  ...
  <div id="floatHead" class="toolbar">
    <div class="l-list">
      <ul class="icon-list">
        ...
        <li  style="display:none;"><asp:LinkButton ID="btnAudit" runat="server" CssClass="save" OnClientClick="return ExePostBack('btnAudit','审核通过后将在前台显示，是否继续？');" onclick="btnAudit_Click"><i></i><span>审核</span></asp:LinkButton></li>
...
<ItemTemplate>
  <tr>
    <td class="comment">
      <div class="title">
        <span class="note"><i><%#Eval("user_name")%></i><i><%#Eval("add_time")%></i><i class="reply"><a href="comment_edit.aspx?id=<%#Eval("id")%>">查看</a></i></span>
        <b><asp:CheckBox ID="chkId" CssClass="checkall" runat="server" /><asp:HiddenField ID="hidId" Value='<%#Eval("id")%>' runat="server" /></b>
        <%#new BuysingooShop.BLL.article().GetTitle(Convert.ToInt32(Eval("article_id")))%>
      </div>
      <div class="ask">
        <%#Convert.ToInt32(Eval("is_lock")) == 1 ? "<b class=\"audit\" title=\"待审核\"></b>" : ""%><%#Eval("reply_content")%>
