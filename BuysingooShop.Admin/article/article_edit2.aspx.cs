...
{
    public partial class article_edit2 : Web.UI.ManagePage
    {
        ...
        private bool DoEdit(int _id)
        {
            ...
            TextBox txtSell_PriceControl = FindControl("field_control_sell_price") as TextBox;
            model.sell_price = Vincent._Convert.ToDecimal(txtSell_PriceControl.Text.Trim(), 0);

            TextBox txtMarket_PriceControl = FindControl("field_control_market_price") as TextBox;
            model.market_price = Vincent._Convert.ToDecimal(txtMarket_PriceControl.Text.Trim(), 0);
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            if (action == Vincent._DTcms.DTEnums.ActionEnum.Edit.ToString()) //修改
            {
                ChkAdminLevel("channel_" + this.channel_name + "_list2", Vincent._DTcms.DTEnums.ActionEnum.Edit.ToString()); //检查权限
                if (!DoEdit(this.id))
                {
                    JscriptMsg("保存过程中发生错误啦！", "", "Error");
                    return;
                }
                JscriptMsg("修改信息成功！", "article_list2.aspx?channel_id=" + this.channel_id, "Success");
            }
            else //添加
            {
                ChkAdminLevel("channel_" + this.channel_name + "_list2", Vincent._DTcms.DTEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误啦！", "", "Error");
                    return;
                }
                JscriptMsg("添加信息成功！", "article_list2.aspx?channel_id=" + this.channel_id, "Success");
            }
        }
        #endregion
