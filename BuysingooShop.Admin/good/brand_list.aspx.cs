...
{
    public partial class brand_list : Web.UI.ManagePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            this.prolistview = Vincent._DTcms.Utils.GetCookie("brand_list_view"); //显示方式
            if (!Page.IsPostBack)
            {
                ChkAdminLevel("channel_" + this.channel_name + "_list", Vincent._DTcms.DTEnums.ActionEnum.View.ToString()); //检查权限
                RptBind("id>0" + CombSqlTxt(this.keywords, this.islock), "sort_id asc,add_time desc,id desc");
            }
        }
        ...
        protected string CombSqlTxt(string _keywords, string _islock)
        {
            ...
            {
                strTemp.Append(" and brandName like '%" + _keywords + "%'");
            }
        ...
        {
            int _pagesize;
            if (int.TryParse(Vincent._DTcms.Utils.GetCookie("brand_page_size"), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    return _pagesize;
        ...
        {
            Response.Redirect(Vincent._DTcms.Utils.CombUrlTxt("brand_list.aspx", "channel_id={0}&keywords={1}&islock={2}",
               this.channel_id.ToString(), this.keywords, ddlLock.SelectedValue));
        }
        ...
        {
            Response.Redirect(Vincent._DTcms.Utils.CombUrlTxt("brand_list.aspx", "channel_id={0}&keywords={1}&islock={2}",
                this.channel_id.ToString(), txtKeywords.Text, this.islock));
        }

        //设置文字列表显示
        protected void lbtnViewTxt_Click(object sender, EventArgs e)
        {
            Vincent._DTcms.Utils.WriteCookie("brand_list_view", "Txt", 14400);
            Response.Redirect(Vincent._DTcms.Utils.CombUrlTxt("brand_list.aspx", "channel_id={0}&keywords={1}&islock={2}&page={3}",
                this.channel_id.ToString(), this.keywords, this.islock, this.page.ToString()));
        }

        //设置图文列表显示
        protected void lbtnViewImg_Click(object sender, EventArgs e)
        {
            Vincent._DTcms.Utils.WriteCookie("brand_list_view", "Img", 14400);
            Response.Redirect(Vincent._DTcms.Utils.CombUrlTxt("brand_list.aspx", "channel_id={0}&keywords={1}&islock={2}&page={3}",
                this.channel_id.ToString(), this.keywords, this.islock, this.page.ToString()));
        }
        ...
        protected void txtPageNum_TextChanged(object sender, EventArgs e)
        {
            ...
            if (int.TryParse(txtPageNum.Text.Trim(), out _pagesize))
            {
                if (_pagesize > 0)
                {
                    Vincent._DTcms.Utils.WriteCookie("brand_page_size", _pagesize.ToString(), 43200);
                }
            }
            Response.Redirect(Vincent._DTcms.Utils.CombUrlTxt("brand_list.aspx", "channel_id={0}&keywords={1}&islock={2}",
                this.channel_id.ToString(), this.keywords, this.islock));
        }
