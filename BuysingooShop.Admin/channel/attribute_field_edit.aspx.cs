...
namespace BuysingooShop.Admin.channel
{
    public partial class attribute_field_edit : Web.UI.ManagePage
    {
        ...
        private bool DoAdd()
        {
            ...
            model.editor_type = Vincent._DTcms.Utils.StrToInt(rblEditorType.SelectedValue, 0);
            model.data_length = Vincent._DTcms.Utils.StrToInt(txtDataLength.Text.Trim(), 0);
            model.data_place = Vincent._DTcms.Utils.StrToInt(ddlDataPlace.SelectedValue, 0);
            ...
            if (bll.Add(model) > 0)
            {
                AddAdminLog(Vincent._DTcms.DTEnums.ActionEnum.Add.ToString(), "添加扩展字段:" + model.title); //记录日志
                result = true;
            }
            return result;
        }
        ...
        private bool DoEdit(int _id)
        {
            ...
            if (bll.Update(model))
            {
                AddAdminLog(Vincent._DTcms.DTEnums.ActionEnum.Add.ToString(), "修改扩展字段:" + model.title); //记录日志
                result = true;
            }
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                ChkAdminLevel("site_channel_field", Vincent._DTcms.DTEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;
