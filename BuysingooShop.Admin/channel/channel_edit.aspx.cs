...
namespace BuysingooShop.Admin.channel
{
    public partial class channel_edit : Web.UI.ManagePage
    {
        ...
        private string GetInherit(string page_type)
        {
            ...
            {
                case "index":
                    result = "BuysingooShop.Web.UI.Page.article";
                    break;
                case "category":
                    result = "BuysingooShop.Web.UI.Page.category";
                    break;
                case "list":
                    result = "BuysingooShop.Web.UI.Page.article_list";
                    break;
                case "detail":
                    result = "BuysingooShop.Web.UI.Page.article_show";
                    break;
            }
            return result;
        }
        ...
        private void ShowInfo(int _id)
        {
            ...
            txtName.Attributes.Add("ajaxurl", "../../tools/admin_ajax.ashx?action=channel_validate&old_channel_name=" + Vincent._DTcms.Utils.UrlEncode(model.name));
        ...
        private bool DoAdd()
        {
            ...
            model.page_size = Vincent._DTcms.Utils.StrToInt(txtPageSize.Text.Trim(), 10);
            ...
            for (int i = 0; i < cblAttributeField.Items.Count; i++)
            {
                ...
                {
                    ls.Add(new Model.channel_field { field_id = Vincent._DTcms.Utils.StrToInt(cblAttributeField.Items[i].Value, 0) });
                }
            }
            ...
            AddAdminLog(Vincent._DTcms.DTEnums.ActionEnum.Add.ToString(), "添加频道" + model.title); //记录日志
            return true;
        }
        #endregion
        ...
        private bool DoEdit(int _id)
        {
            ...
            for (int i = 0; i < cblAttributeField.Items.Count; i++)
            {
                if (cblAttributeField.Items[i].Selected)
                {
                    ls.Add(new Model.channel_field { channel_id = model.id, field_id = Vincent._DTcms.Utils.StrToInt(cblAttributeField.Items[i].Value, 0) });
                }
            }
            model.channel_fields = ls;
            ...
            AddAdminLog(Vincent._DTcms.DTEnums.ActionEnum.Edit.ToString(), "修改频道" + model.title); //记录日志
            return true;
        }
        #endregion
        ...
        protected void btnSubmit_Click(object sender, EventArgs e)
        {
            ...
            {
                ChkAdminLevel("site_channel_list", Vincent._DTcms.DTEnums.ActionEnum.Add.ToString()); //检查权限
                if (!DoAdd())
                {
                    JscriptMsg("保存过程中发生错误！", "", "Error");
                    return;
