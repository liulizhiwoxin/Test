...
namespace BuysingooShop.DAL
{
    public partial class plugin
    {
        ...
        public Model.plugin GetInfo(string dirPath)
        {
            ...
            if (!File.Exists(dirPath + Vincent._DTcms.DTKeys.FILE_PLUGIN_XML_CONFING))
                return aboutInfo;

            XmlDocument xml = new XmlDocument();
            xml.Load(dirPath + Vincent._DTcms.DTKeys.FILE_PLUGIN_XML_CONFING);
            try
            {
        ...
        {
            XmlNodeList xnList = _Xml.ReadNodes(dirPath + Vincent._DTcms.DTKeys.FILE_PLUGIN_XML_CONFING, xPath);
            foreach (XmlElement xe in xnList)
            {
                if (xe.NodeType != XmlNodeType.Comment && xe.Name.ToLower() == "rewrite")
                {
                    ...
                    {
                        //生成模板文件
                        Vincent._DTcms.PageTemplate.GetTemplate(sitePath, tempPath, skinName, xe.Attributes["templet"].InnerText, xe.Attributes["page"].InnerText, xe.Attributes["inherit"].InnerText, Vincent._DTcms.DTKeys.DIRECTORY_REWRITE_PLUGIN, 1);
                    }
                }
            }
        }
        ...
        {
            return _Xml.UpdateNodeInnerText(dirPath + Vincent._DTcms.DTKeys.FILE_PLUGIN_XML_CONFING, xPath, value);
        }
        ...
        {
            bool result = true;
            List<string> ls = ReadChildNodesValue(dirPath + Vincent._DTcms.DTKeys.FILE_PLUGIN_XML_CONFING, xPath);
            if (ls != null)
            {
        ...
        private List<string> ReadChildNodesValue(string filePath, string xPath)
        {
            try
            {
                ...
                XmlNodeList xnList = _Xml.ReadNodes(filePath, xPath);
                if (xnList.Count > 0)
                {
                    foreach (XmlElement xe in xnList)
                    {
