...
namespace BuysingooShop.DAL
{
	...
	public partial class user_login_log
	{
        ...
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append("select count(*) as H from " + databaseprefix + "user_login_log ");
            if (strWhere.Trim() != "")
            {
        ...
        /// 根据用户名返回上一次登录记录
        /// </summary>
        public Model.user_login_log GetModel(string user_name, int top_num)
        {
            ...
            {
                StringBuilder strSql = new StringBuilder();
                strSql.Append("select top 1 id from (select top " + top_num + " id from " + databaseprefix + "user_login_log where user_name=@user_name order by id desc) as T ");
